...
test.describe('APIProduct RBAC - no permission user', () => {
  ...
  test('list page shows no-permission view', { tag: ['@smoke', '@kuadrant'] }, async ({ page }) => {
  ...
  test('definition tab shows access denied', { tag: ['@smoke', '@kuadrant'] }, async ({ page }) => {
  ...
  test('policies tab shows access denied', { tag: ['@smoke', '@kuadrant'] }, async ({ page }) => {
    await impersonateAndNavigate(
      page,
      `/kuadrant/apiproducts/ns/${TEST_NAMESPACE}`,
