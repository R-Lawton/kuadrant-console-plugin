...
test.describe('Policy Topology', () => {
  ...
  test('renders the topology graph with fixture resources', { tag: ['@smoke', '@core'] }, async ({ page }) => {
  ...
  test('filters nodes by resource type', { tag: ['@smoke', '@core'] }, async ({ page }) => {
    await expect(nodeByLabel(page, ROUTE_NODE_LABEL).first()).toBeVisible({ timeout: 20_000 });
  ...
  test('clearing all resource filters empties the graph', { tag: ['@nightly', '@core'] }, async ({ page }) => {
  ...
  test('filters nodes by namespace', { tag: ['@smoke', '@core'] }, async ({ page }) => {
  ...
  test('node context menu navigates to the resource details page', { tag: ['@smoke', '@core'] }, async ({ page }) => {
