...
test.describe('APIProduct Overview Tab', () => {
  ...
  test('should display About section with all fields and correct styling', { tag: ['@nightly', '@kuadrant'] }, async ({ page }) => {
  ...
  test('should display authentication methods with "Not set" and configured states', { tag: ['@nightly', '@kuadrant'] }, async ({
    page,
  }) => {
    // Test "Not set" state
  ...
  test('should display plan tiers with "Not set" and configured states', { tag: ['@nightly', '@kuadrant'] }, async ({ page }) => {
    // Test "Not set" state
    await navigateToAPIProductOverview(page);
    await expect(page.locator('text=About').first()).toBeVisible({ timeout: 10_000 });
  ...
  test('should display Documentation section with links, "Not set" states, and edit buttons', { tag: ['@nightly', '@kuadrant'] }, async ({
    page,
  }) => {
  ...
  test('should display Contact section with links, "Not set" states, and edit buttons', { tag: ['@nightly', '@kuadrant'] }, async ({
    page,
  }) => {
    // Test "Not set" states
    await navigateToAPIProductOverview(page, TEST_NAMESPACE, 'test-product-no-auth');
  ...
  test('should open and close edit modals', { tag: ['@nightly', '@kuadrant'] }, async ({ page }) => {
    await navigateToAPIProductOverview(page);
    await expect(page.locator('text=About').first()).toBeVisible({ timeout: 10_000 });
  ...
  test('should navigate to Overview tab from detail page', { tag: ['@nightly', '@kuadrant'] }, async ({ page }) => {
  ...
  test('should display all sections in correct order', { tag: ['@nightly', '@kuadrant'] }, async ({ page }) => {
    await navigateToAPIProductOverview(page);
    await expect(page.locator('text=About').first()).toBeVisible({ timeout: 10_000 });
