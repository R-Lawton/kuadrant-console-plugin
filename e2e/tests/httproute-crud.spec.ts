...
`);
  ...
  test('creates an HTTPRoute via the form', { tag: ['@nightly', '@gateway'] }, async ({ page }) => {
  ...
  test('edits an existing HTTPRoute', { tag: ['@nightly', '@gateway'] }, async ({ page }) => {
    const routeName = `e2e-httproute-${uid()}`;
    applyResource(`
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
...
`);
  ...
  test('form↔YAML sync preserves data', { tag: ['@nightly', '@gateway'] }, async ({ page }) => {
    const routeName = `e2e-httproute-${uid()}`;
    await gotoPage(page, `/k8s/ns/${namespace}/gateway.networking.k8s.io~v1~HTTPRoute/~new`);
